...
";
    ...
    [MenuItem("Tools/SceneLoader/EnsureMissingScenes")]
    static private void OnProjectLoadedInEditor()
    {
        // Get list of all scenes
        List<SceneInfo> allScenes = GetAllScenesAtPath("/External(Plugin-ins)/HighlightingSystem/HighlightingSystemDemo/Scenes");
    ...
    [MenuItem("HighlightingSystem/Readme")]
    static private void Readme()
    {
        Debug.Log("你可以通过 Tools -> SceneLoader -> EnsureMissingScenes 添加默认测试场景 HighlightingSystemDemo !!");
        EditorUtility.DisplayDialog("Highlighting System : SceneLoader", "你可以通过 Tools -> SceneLoader -> EnsureMissingScenes 添加默认测试场景 HighlightingSystemDemo !!", "确定");
    }
